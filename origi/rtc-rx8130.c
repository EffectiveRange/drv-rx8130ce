...
static const struct i2c_device_id rx8130_id[] = {{"rx8130", 0}, {}};
...
static int rx8130_probe(struct i2c_client *client,
  ...
  rx8130->rtc = rtc_device_register(client->name, &client->dev, &rx8130_rtc_ops,
                                    THIS_MODULE);
  ...
  if (client->irq > 0) {
    ...
    err = devm_request_threaded_irq(&client->dev, client->irq, NULL, rx8130_irq,
